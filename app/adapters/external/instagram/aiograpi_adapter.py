...
class AiograpiAdapter:
    ...
    async def get_thread(self, thread_id: int) -> list[InstagramThread]:
        ...
        thread.reverse()
        to_exclude = {"id", "user_id", "thread_id", "timestamp", "item_type", "is_shh_mode", "reactions", "reply", "link", "animated_media", "media", "visual_media", "media_share", "reel_share", "story_share", "felix_share", "xma_share", "clip", "placeholder"}  # noqa: E501
        clean_thread = []
        clean_thread.extend(InstagramThread(**message.model_dump(exclude=to_exclude)) for message in thread)
        return clean_thread
